...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const update = () => { frameRef.current = null; const lifecycle = lifecycleRef.current; if (!lifecycle) return; const start = lifecycle.offsetTop; const distance = Math.max(1, lifecycle.offsetHeight - window.innerHeight); const raw = clamp((window.scrollY - start) / distance, 0, 0.9999); const timeline = raw < 0.82 ? (raw / 0.82) * 0.96 : 0.98; let nextProgress = timeline; if (media.matches) { const phaseIndex = phases.findIndex((item) => timeline >= item.start && timeline < item.end); nextProgress = timeline >= 0.96 ? 0.98 : discreteAnchors[Math.max(0, phaseIndex)]; } if (Math.abs(nextProgress - lastProgressRef.current) < 0.0005) return; lastProgressRef.current = nextProgress; setProgress(nextProgress); };
    ...
    return () => { media.removeEventListener("change", syncMotion); window.removeEventListener("scroll", schedule); window.removeEventListener("resize", schedule); if (frameRef.current !== null) window.cancelAnimationFrame(frameRef.current); };
  }, []);
  ...
  return <main>
    ...
    <section className="lifecycle" id="lifecycle" ref={lifecycleRef} data-phase={phase.key} data-reduced-motion={reducedMotion}><SpatialFrame className="sticky-story"><div className="story-copy" aria-live="polite"><p className="chapter">{String(phases.indexOf(phase) + 1).padStart(2, "0")} / 07</p><p className="moment">{phase.eyebrow}</p><h2>{phase.title}</h2><p className="body-copy">{phase.body}</p></div><CalmSurface active={phase.key === "nothing" || phase.key === "protection"}><div className="lifecycle-visual"><KynloLifecycleRing progress={progress} phase={phase} /><OwnershipPath reveal={successionReveal} /></div></CalmSurface><TimeRail stages={phases} activeKey={phase.key} /><div className="lifecycle-progress" aria-hidden="true"><span style={{ transform: `scaleX(${progress})` }} /></div></SpatialFrame></section>
